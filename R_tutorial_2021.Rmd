...
data_cobre %>%
  ...
  group_by(First_degree_relative_psychosis, .drop = TRUE) %>%
...
````{r, size="tiny"}
data_cobre %>%
  mutate(Time_since_onset = Age - Age_onset_psychiatric_illness) %>%
  group_by(Gender, .drop = TRUE) %>%
  summarize(mean = mean(Time_since_onset, na.rm = TRUE))
```
...
````{r, size="tiny"}
data_cobre %>%
  summarize(min = min(Age_onset_psychiatric_illness, na.rm = TRUE),
            max = max(Age_onset_psychiatric_illness, na.rm = TRUE))
```
...
data_cobre %>%
  filter(Study_group != "Control") %>%
  group_by(Study_group, .drop = TRUE) %>%
  summarize(median = median(Age_onset_psychiatric_illness, na.rm = TRUE))
```