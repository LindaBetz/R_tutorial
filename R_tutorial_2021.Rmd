...
```
# Introduction
![](./imgs/hello.jpg){ width=50% height=50% }

# Structure of the tutorial
- June 28 (today): Introduction to R & Data Wrangling
- June 29: Recap of ML concepts, Hands-On ML Analyses
...
- We'll use neuroimaging data later for Machine Learning 
```{r, message = TRUE, size="tiny"}
data_cobre 
```
...
```{r, size="tiny"}
filter(data_cobre, study_group %in% c(16, 18))
```
...
````{r, size="tiny"}
data_cobre %>% 
  group_by(study_group, gender) %>% # save grouped data frame here
...
````{r, size="tiny"}
data_cobre %>%
transmute(age, young_participant = if_else(age < median(age, na.rm = TRUE), true = "yes", false = "no")) 
```
...
In the upcoming days, we will walk you through a Machine Learning analysis using neuroimaging data.
