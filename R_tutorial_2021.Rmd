...
`filter()` allows you to subset observations based on their values. The first argument is the name of the data frame. The second and subsequent arguments are the expressions that filter the data frame. For example, we can select all participants with an age of 65 years.
```{r, size="tiny"}
...
- Select all variables which contain the word "study" or "age".
...
# Solutions
Select all variables that contain the word "study" or "age".
```{r, size="tiny"}
