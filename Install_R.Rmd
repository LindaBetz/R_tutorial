...
RStudio is a GUI that makes writing and organizing your R code a lot easier. The RStudio interface looks the same for Windows, Mac OS, and Linux. 

You can download RStudio for free (https://rstudio.com/products/rstudio/). Install the Desktop version of R studio (Open Source Edition). Click on the corresponding button ("Download RStudio Desktop") and follow the simple instructions that follow. Once you’ve installed RStudio, you can open it like any other program on your computer—usually by clicking an icon on your desktop.

# Opening RStudio

When you open RStudio, a window appears with three panes in it. It should look something like this:


![](./imgs/rstudio.png){ width=90% height=90% }



# How to install packages
Many useful functions do not come preloaded when you start R, but reside in packages that can be installed on top of R. For the tutorial, please install some packages in advance ("tidyverse", "caret", "e1071", "mlr"). To do that, type 
```{r, eval=FALSE}
install.packages(c("tidyverse", "caret", "e1071", "mlr"))
```
in the console in RStudio, and execute the code. Note that you need a connection to the internet to install packages. 

# Test if the installation worked
Test if installation worked by typing in the console: `library(tidyverse)`. 
If everything worked fine, the output in the console look something like this:
```{r, size="tiny"}
...
# Problems
If something did not work (e.g., you get an error of the nature `Error in library(tidyverse) : there is no package called ‘tidyverse’`), try to install the packages manually by clicking Tools → Install packages in RStudio. If you still cannot get things running, please contact us **before the tutorial** (linda.betz@uk-koeln.de) :)
