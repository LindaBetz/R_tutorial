...
Many useful functions do not come preloaded when you start R, but reside in packages to be installed on top of R. For the tutorial, please install some packages in advance ("tidyverse", "reshape2", "caret", "e1071", "mlr"). To do that, open RStudio and type 
```{r, eval=FALSE}
install.packages(c("tidyverse", "reshape2", 
                   "caret", "e1071", "mlr"))
```
at the command prompt (`>`) of the console and press enter to execute the code. In case you're asked whether you would also like to install dependencies, choose "yes". Some messages will scroll by, and there will be a pause where nothing appears to happen (but the installation is actually occurring). At last, the output parade should end with a message like:
