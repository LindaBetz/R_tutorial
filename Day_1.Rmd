...
```
# Introduction
![](./imgs/hello.jpg){ width=40% height=40% }

# Structure of the tutorial
- June 28 (today): Introduction to R & Data Wrangling
- June 29: Practical Aspects of Data Wrangling, Hands-On ML Analyses
...
- Redundant features (e.g. sum scores)
- Observations with many missing values 
...
- Contains clinical participants with a diagnosis with schizophrenia, and healthy controls
- Every row represents one participant, every column is a variable 
- We'll use neuroimaging data tomorrow for Machine Learning 
```{r, message = TRUE, size="tiny"}
data_cobre 
```
...
# Data transformation
We are going to look at four key dplyr functions that allow you to solve the vast majority of data manipulation challenges:
...
# Logical operations
The following code finds all participants that are either 16 or 18 years old:

```{r, size="tiny"}
filter(data_cobre, age == 16 | age == 18)
```
...
```{r, size="tiny"}
filter(data_cobre, age %in% c(16, 18))
```
...
# Break
After the break, we'll get started with some exercises.
\
\
...
# Exercises: Getting started
![](./imgs/tutorial_start.png){width=90% height=90%}
...
Find all men who are 22, 25 or 27 years old. How many are there?
```{r, size="tiny"}
filter(data_cobre, gender == "male" & age %in% c(22, 25, 27))
```

# Solutions
How many patients have a missing value in the variable `age_onset_psychiatric_illness`?
```{r, size="tiny"}
filter(data_cobre, study_group == "schizophrenia" & is.na(age_onset_psychiatric_illness))
```
...
summarize(by_group, age = mean(age)) # mean age per group
```
...
````{r, size="tiny"}
by_group <- group_by(data_cobre, study_group, gender) # save grouped data frame here
mean_age_groups <- summarize(by_group, age = mean(age)) # mean age per group
filtered_age <- filter(mean_age_groups, age > 35)
```
...
````{r, size="tiny"}
data_cobre %>%
  group_by(study_group, gender) %>%
  summarize(age = mean(age), .groups = "drop") %>%
  filter(age > 35)
```
...
````{r, size="tiny"}
data_cobre %>% 
  group_by(study_group, first_degree_relative_psychosis) %>% 
  summarize(age_mean = mean(age), age_sd = sd(age)) 
```
...
# `if_else` explained
`if_else` is very useful for variable transformations. It tests if a condition is true, and replaces values in the variable depending on whether the condition is true or false.
````{r}
(x <- c(NA, seq(from = -1, to = 1)))
if_else(x < 0, "negative", "positive", "missing")
```
Note that when testing multiple conditions, the code is more readable and less error-prone if we use `case_when`.
...
````{r, size="tiny"}
data_cobre %>%
  transmute(age, young_participant = if_else(
...
````{r, size="tiny"}
data_cobre %>%
  filter(study_group == "schizophrenia" &
...
# Summary
- Inspect & clean your data before starting any analysis. With messy data, we have a "garbage in, garbage out" scenario in ML.
- Most issues stem from (unstructured) data collection/documentation and loading the data into R (→ errors in variable type conversion).
- The `tidyverse` offers many operations for efficient data wrangling. 

\
\
![](./imgs/data_cleaning.jpg){ width=45% height=45% }
...
In the next days, we will talk about practical aspects of data wrangling and walk you through a Machine Learning analysis using neuroimaging data.

# References
- R for Data Science: https://r4ds.had.co.nz/
- Open dataset: https://coins.trendscenter.org/ (you need to create an account if you want to download data)
